...
The class :py:class:`pystructrans.Lattice` handles the generic lattice objects.
...
There, the definition :eq:`lattice-1` is equivalent to
...
.. autoclass:: pystructrans.Lattice
    :members: getbase, getdimension, getpointgroup, getLauegroup,
              getlatticegroup, getspeciallatticegroup
    ...
    .. automethod:: pystructrans.Lattice.inpointgroup

        An orthogonal matrix :math:`\mathbf Q` is in the `point group` of the lattice
        :math:`\mathcal L(\mathbf E)` if and only if :math:`\mathcal L(\mathbf Q\mathbf E) = \mathcal L(\mathbf E)`


.. .. doctest::
    >>> e1 = lat.gete1(); e2 = lat.gete2();
    >>> L = np.array([[1,-1,0],[1,0,0],[0,0,1]])
    >>> lat.transformBase(L)
    >>> print(lat.getBase())
    [[ 2. -1.  0.]
     [ 0.  1.  0.]
     [ 0.  0.  1.]]
    >>> print(lat.gete1() == e1 + e2)
    [ True  True  True]

.. .. doctest::
    >>> L = np.array([[1.0,-1,0],[1,0,0],[0,0,1]])
    >>> print(L.dtype)
    float64
    >>> lat.transformBase(L)
    Traceback (most recent call last):
    LatticeError: Base transformation matrix must be an integeral matrix.
    >>> L = L.astype('int')
    >>> lat.transformBase(L)
    >>> print(lat.getBase())
    [[ 1. -2.  0.]

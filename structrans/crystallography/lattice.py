...
from structrans.crystallography.matrix_group import MatrixGroup, CUBIC_LAUE_GROUP, HEX_LAUE_GROUP, SQUARE_GROUP, SQUARE_GROUP_EXT
...
def _in_O3(Q):
    ...
    if not _is_square_mat(Q):
        ...
        if Q.dtype == np.int:
            ...
            return abs(la.det(Q)) == 1.0 and np.allclose(np.dot(Q, Q.T), np.eye(len(Q)))